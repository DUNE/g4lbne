...
//  fTargetDownstrCanFlangeThick = (3.0/8.0)*in; ??? mistake See above 
  ...
  // Content of the downstream tip of the target containment tube 
  fTargetCTubeReturnLengthDownstr = fTargetSLengthDownstrEnd - 2.0*mm;
  // The above qantity is obsolete. A better and simplfied model 
  // based on 3 Tubs one titanium, followed by water, followed by titanium
  ...
  const double lEffCTubeReturnDownstr = 2.0*(0.6*in) + 15*mm;  // 1rst term is the bend, horizontal part, top & bottom. 
  ...
  fTargetCTubeReturnDownstrRadInner = 11.0*mm; // Rougly 14.5 - 3 - a bit, to make up the mass 
                                               // without increasing the thickness by too much
  fTargetCTubeReturnDownstrRadOuter = 14.25*mm;
...
//               " Water " << fTargetCTubeReturnDownstrThickWater << std::endl;
  ...
  fTargetHeContTubeInnerRadius = 30.0*mm/2.0 - fTargetHeContTubeThickness; // Doc DB 6100, page 1 
  ...
  fTargetAlignRingOuterRadius = fTargetHeContTubeInnerRadius - 0.1*mm;
...
//
     fAbsorberGDMLFilename = G4String("./gdml/abs_geomSept13.gdml");
...
//  fTargetHeContTubeLengthInHorn = fTargetLengthIntoHorn + fTargetSLengthDownstrEnd;
  fTargetHeContTubeLengthInHorn = fTargetLengthIntoHorn + 0.025*mm;
...
// 
    const double totalLengthHorn1 = fHorn1DownstreamPlateLength + fHorn1Length + 0.5*cm;
    const double totalLengthUpstr = fTargetAndBaffleLengthApprox + 0.5*cm;
    info.fParams[2] = totalLengthHorn1 + totalLengthUpstr; // so far, we changed nothing. 
     
    info.fPosition[2] = (info.fParams[2]/2 - totalLengthUpstr + 0.5*cm); // Definiing MCZERO, Drawing 8875. 112-MD-363097, annotated by jim Hylen
    std::cerr << " Placing TargetHallAndHorn1, totalLengthHorn1 " << totalLengthHorn1 
               << " totalLengthUpstr " << totalLengthUpstr << " ZShift " << info.fPosition[2] << std::endl;
    info.fTypeName = G4String("Box");
    ...
    info.fParams[0] = 64*in - 1.0*cm;
    info.fParams[1] = 60*in - 1.0*cm; // approximate, we need room for the rotation (Horn is tilted with respect to 
                                      // true horizontal 
	 ...
	 maxRadIOHorn1 += 2.0*cm;    
       ...
       if (name.find("TargetUpstrDownstr") != std::string::npos) {
          ...
          if (name == G4String("TargetUpstrDownstrSegment")) { // 
            info.fParams[0] = fTargetFinWidth + 0.05*mm; 
  ...
  if (name.find("Horn1") == 0) { 
    if (name == G4String("Horn1Hall")) {
      const LBNEVolumePlacementData *plInfoM = Find(name, G4String("TargetHallAndHorn1"), G4String("Create"));
       ...
       info.fParams[0] = 64.0*in - 1.0*cm;
       info.fParams[1] = 50.0*in - 1.0*cm;
 ...
 // despite being a single copy, do the placement in a separate method. 	    
     ...
     if (name == G4String("Horn1TargetCoolingTubeDwnRetWater")) { //           
	    ...
	    info.fParams[4] = fTargetCTubeReturnDownstrCutAngleSize;
	       ...
	       fTargetCTubeReturnDownstrThickWater/2. - fTargetCTubeReturnDownstrThickTitanium/2 - 1.5*mm  ; // leave room for the cap. 
     }
...
//  Horizontal fin for beam alignment. Most important thing in the canister, in terms of material budget 
    ...
    LBNEVolumePlacementData *plTargetFinCorUpLeft = Create(G4String("TargetFinVertCornerUpLeft"));
    LBNEVolumePlacementData *plTargetFinCorUpRight = Create(G4String("TargetFinVertCornerUpRight"));
    LBNEVolumePlacementData *plTargetFinCorDwnLeft = Create(G4String("TargetFinVertCornerDwnLeft"));
    LBNEVolumePlacementData *plTargetFinCorDwnRight = Create(G4String("TargetFinVertCornerDwnRight"));
...
//      std::cerr << " Placing target at Zs " << 	posTmp[2] << std::endl;			  
    ...
    posTmp[2] = 0.;    
    new G4PVPlacement((G4RotationMatrix *) 0, 
	                            posTmp, plTargetFinCorUpLeft->fCurrent, 
				          nameTgtUpDownSeg+G4String("TargetFinVertCornerUpLeft_P"), 
				          vSeg->GetLogicalVolume(), false, 0, fCheckVolumeOverLapWC);
    posTmp[0] = fTargetFinWidth/4.; posTmp[1] = fTargetFinHeight/2. -  fTargetCTubeOuterRadius/2. + 0.125*mm;
    new G4PVPlacement((G4RotationMatrix *) 0, 
	                            posTmp, plTargetFinCorUpRight->fCurrent, 
				          nameTgtUpDownSeg+G4String("TargetFinVertCornerUpRight_P"), 
				          vSeg->GetLogicalVolume(), false, 0, fCheckVolumeOverLapWC);
					  
    posTmp[0] = -fTargetFinWidth/4.; posTmp[1] = -1.0*(fTargetFinHeight/2. -  fTargetCTubeOuterRadius/2. + 0.125*mm);
    new G4PVPlacement((G4RotationMatrix *) 0, 
	                            posTmp, plTargetFinCorDwnLeft->fCurrent, 
				          nameTgtUpDownSeg+G4String("TargetFinVertCornerDwnLeft_P"), 
				          vSeg->GetLogicalVolume(), false, 0, fCheckVolumeOverLapWC);
    posTmp[0] = fTargetFinWidth/4.; posTmp[1] = -1.0*(fTargetFinHeight/2. -  fTargetCTubeOuterRadius/2. + 0.125*mm);
    new G4PVPlacement((G4RotationMatrix *) 0, 
	                            posTmp, plTargetFinCorDwnRight->fCurrent, 
				          nameTgtUpDownSeg+G4String("TargetFinVertCornerDwnRight_P"), 
				          vSeg->GetLogicalVolume(), false, 0, fCheckVolumeOverLapWC);
   				  
    // Repeat the last few mantra for the last segments 
    if (fTargetFinLengthSplitUpstr > 0.3*mm) {
